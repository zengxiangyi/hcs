...
- 账号注册（`/api/auth/register`，用户名唯一 + 手机号/邮箱格式校验）
- 忘记密码（手机号 + 邮箱身份验证后重置）
...
| CORS | @fastify/cors | ^10.0.1 | 允许跨域（反射请求来源 + 凭证） |
...
| 数据库 | **MySQL** | — | 默认库兜底 `user_test`（utf8mb4），可由 `DB_NAME` 覆盖 |
| 参数校验 | **Zod** | ^3.24.1 | 请求体 / 查询参数校验 |
| 环境变量 | dotenv | ^16.4.5 | 读取 `.env` |
| 密码哈希 | md5（`utils/md5.ts`，基于 Node 内置 `node:crypto`） | — | 密码存储与比对（历史兼容契约） |
| 测试 | Node 内置 `node:test` + `node:assert/strict` | — | 仅 `utils/__tests__/md5.test.ts` 契约测试 |
| 迁移工具 | drizzle-kit | ^0.30.1 | generate / push / studio |
| 开发运行 | tsx | ^4.19.2 | TS 直接运行（watch 热重载） |
| 语言 | TypeScript | ^5.7.2 | 严格模式 |

> 当前**已引入** `@fastify/jwt` 与 `md5` 哈希，登录鉴权已具备真实 JWT + 中间件拦截；测试使用 Node 内置 `node:test`（仅 md5 契约测试，`npm test`）。
> 未引入：vitest/jest 等第三方测试框架、日志分析工具、刷新令牌 / 细粒度权限控制、Dockerfile。
...
|------|------|------|------|------|
| id | `id` | int | PK, autoincrement | 自增主键 |
| username | `username` | varchar(64) | not null, unique | 用户名（唯一键由 DB 层保证） |
| password | `password` | varchar(128) | not null | 密码（`md5(...)` 哈希存储，直存直比） |
| name | `name` | varchar(64) | nullable | 显示名（注册时默认取用户名） |
...
- 注册 `@fastify/cors`（`origin: true` 反射请求来源 + `credentials: true`；因部署来源暂不确定，待确定前端域名后再收紧为白名单）。
- 注册 `@fastify/jwt`（`sign.expiresIn: '8h'`）。
  - **`JWT_SECRET` 为必填环境变量**：缺失时打印错误并 `process.exit(1)` **拒绝启动**，不回退到硬编码默认值（避免 token 可被伪造）。
...
- 健康检查 `GET /api/health`，返回 `success({status:'ok'})`。
- **统一错误处理**（`app.setErrorHandler<FastifyError>`）：
  - `ZodError` → `400`，msg 为 `error.issues.map(i => i.message).join('; ')`；
  - `error.statusCode` 存在且 `< 500` → 原状态码透传（含 msg）；
  - 其余 → `500 {msg:'服务器内部错误'}` 兜底。
  - 所有错误用 `fail(code, msg)` 包装。
...
- 连接参数取自环境变量，均有默认值兜底（`DB_NAME` 缺省 `user_test`）。

### 5.4 鉴权路由 `src/routes/auth.ts`

- `POST /api/auth/register`，body `{username, password, cellphone, email}`，Zod 校验：用户名非空、密码 ≥ 6 位、手机号 `^1[3-9]\d{9}$`、邮箱格式。
  - 用户名唯一性由 `accounts.username` 的 UNIQUE 约束在 DB 层保证（避免先查后插竞态窗口）；并发冲突统一捕获为 `400 {msg:'用户名已存在'}`，不泄露底层错误。
  - 密码契约为「直存直比」：前端已提交 md5 密文，后端原样存储（与登录比对一致）。
...
- `POST /api/auth/verify-identity`，body `{cellphone, email}`：按手机号 + 邮箱匹配账号，成功返回 `success(null,'验证成功')`，失败 `400 {msg:'验证信息错误'}`。
- `POST /api/auth/reset-password`，body `{cellphone, email, newPassword}`：身份验证通过后按账号 id 更新密码（newPassword ≥ 6 位，前端按约定提交 md5 后密文）；验证失败 `400 {msg:'验证信息错误'}`，成功 `success(null,'密码重置成功')`。
- `verify-identity` 与 `reset-password` 共用 `findAccountByContact()`（只查 `id`，供存在性判断与按 id 更新复用）。

> 鉴权由 `index.ts` 的全局 preHandler 统一拦截，无需在每个路由内单独校验（`/api/auth/*` 全部接口在白名单内跳过，含 register）。
...
- **详情** `GET /api/user/info`：返回表中第一条用户（对齐前端 `getUserInfo` 需求）；空表返回 `404 {msg:'暂无用户数据'}`。
- **新增** `POST /api/users`：body `{userName, roleName, department, state}`，自动填充 `createTime` 为当前时间字符串，返回最新记录 `msg:'新增成功'`。
- **修改** `PUT /api/users/:id`：body 同上，更新除 id/createTime 外的字段，返回最新记录 `msg:'修改成功'`。
- **删除** `DELETE /api/users/:id`：删除成功返回 `success(null,'删除成功')`。
- `:id` 参数校验：`parseIdOr400()` 将 `:id` 转为整数，非正整数 → `400 {msg:'无效的用户 ID'}`；`findUserOr404()` 查不存在 → `404 {msg:'用户不存在'}`（修改/删除共用「查存在 → 404」逻辑）。
...
| POST | `/api/auth/register` | body `{username,password,cellphone,email}` | 注册新账号（免 token；用户名冲突返 400） |
...
| GET | `/api/user/info` | — | 查询单条用户（取第一条，需 token；空表 404） |
| POST | `/api/users` | body `{userName,roleName,department,state}` | 新增用户（需 token） |
| PUT | `/api/users/:id` | body `{userName,roleName,department,state}` | 修改用户（需 token；非法/不存在 id 返 400/404） |
| DELETE | `/api/users/:id` | — | 删除用户（需 token；非法/不存在 id 返 400/404） |
...
cp .env.example .env      # 修改 DB_HOST/DB_PORT/DB_USER/DB_PASSWORD/DB_NAME
                          # JWT_SECRET 必填：缺失时服务拒绝启动（不回退默认值）

# 建表（需数据库已存在；注意 DB_NAME 需与建库名一致——
#   代码兜底默认 user_test，.env.example 示例值为 page，两者不一致时以 .env 为准。
#   不存在则先执行：CREATE DATABASE <DB_NAME> CHARACTER SET utf8mb4;）
npm run db:push
...
登录测试：`POST /api/auth/login` body `{"username":"admin","password":"e10adc3949ba59abbe56e057f20f883e"}`（该值为 `md5('123456')`；前端按契约提交 md5 密文，种子账号密码经 `md5('123456')` 存储）。
...
| `npm test` | 运行 `utils/__tests__/*.test.ts`（node:test，当前仅 md5 契约测试） |
...
- **密码约定**：前端登录 / 注册 / 重置密码提交的是 `md5(password)` 后的密文，与后端 `accounts.password` 中存储的 md5 密文直接比对（直存直比）。
...
3. **新增免校验接口**：若新接口需跳过 JWT 鉴权，在 `index.ts` 的 `preHandler` 白名单（`/api/auth/` 前缀）或 `PUBLIC_PATHS` 中补充。
4. **JWT_SECRET 必填**：`index.ts` 启动时强校验 `JWT_SECRET`，缺失即拒绝启动；不要在代码中回退硬编码默认密钥。
5. **密码处理**：新增账号（含 register）/ 改密码务必遵循「直存直比」契约——前端提交 md5 密文、后端原样存储，与 `utils/md5.ts` 保持一致。⚠️ 注意 md5 仅为历史兼容性包袱（非规范），仅用于维护现有契约，新功能不得新引入 md5 散列。
6. **保持契约一致**：任何响应改动务必维持 `{ code, data, msg }` 结构，否则前端拦截器会误判；登录失败保持 `400`。
7. **安全升级前**：md5 仅为演示强度，生产部署前建议升级密码哈希算法并补充权限控制。
