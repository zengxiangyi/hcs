...
7. **Jackson 属性名陷阱（2026-09-05 评审实证）**：JSON 字段名以 Java Bean 属性名为准，**不等于 DB 列名**——`flownode` 表的 `X/Y/W/H` 四个大写列，经 `getX()` 等 getter 序列化后 JSON 键是**小写 `x/y/w/h`**（Jackson 3 `stdManglePropertyName`），前端 TS 类型必须用小写（见 `frontend/src/api/flowNode.ts`）。后端实体字段为 varchar 时 JSON 是字符串，前端不要声明成 `number`（如 `blueprint.weight`、`flowGraph.width/height`、`transferorder.num/weight`）。
