...
5. **数据库**：MySQL 库 `page`；列名/表名一律**小写无下划线**（Linux 下表名大小写敏感，SQL 写驼峰表名会报 `Table 'page.flowNode' doesn't exist`）；`ddl-auto=none`，**所有 DDL/DML 由 DBA 执行，AI 只以 SQL 文本交付，禁止自行连库执行**。schema 真源 = `backend/docs/tables.md`。
